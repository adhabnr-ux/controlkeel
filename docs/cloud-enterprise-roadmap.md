...
Every named phase and stretch branch on this roadmap has shipped code with passing tests. **1749 tests, 0 failures** at HEAD. Each row below points at the implementing module(s) so reviewers can verify the trust-boundary claims directly.
...
| **Phase 3 — Hosted MCP gateway** | ✓ shipped | Tool-call audit log ([lib/controlkeel/cloud/mcp_audit_log.ex](../lib/controlkeel/cloud/mcp_audit_log.ex)); per-tool policy ([lib/controlkeel/cloud/mcp_policy.ex](../lib/controlkeel/cloud/mcp_policy.ex)); supply-chain registry ([lib/controlkeel/cloud/mcp_registry.ex](../lib/controlkeel/cloud/mcp_registry.ex)); content guardrails ([lib/controlkeel/cloud/guardrails.ex](../lib/controlkeel/cloud/guardrails.ex)); all gated through [lib/controlkeel/protocol_interop.ex](../lib/controlkeel/protocol_interop.ex) `authorize_hosted_tool_call/4`; **per-workspace tool catalogs** ([lib/controlkeel/accounts/workspace_tool_policy.ex](../lib/controlkeel/accounts/workspace_tool_policy.ex)) — `inherit`/`allowlist`/`denylist` modes, gated in `authorize_hosted_tool_call`, CLI `workspace tool-policy get/set`, API `GET/PUT /workspaces/:id/tool-policy`; **signed skill pipeline** ([lib/controlkeel/cloud/skill_attestation.ex](../lib/controlkeel/cloud/skill_attestation.ex)) — Ed25519 cert verification via `:crypto`, wired into `McpRegistry.lookup` via `cert:` opt |
| **Phase 4 — Team workspace** | ✓ shipped | Accounts context ([lib/controlkeel/accounts.ex](../lib/controlkeel/accounts.ex)); workspace↔org linkage; review assignment + decision + audit trail; org budget rollup + cap enforcement at proxy ([lib/controlkeel/proxy/governor.ex](../lib/controlkeel/proxy/governor.ex)); admin CLI (`org create/list/budget set/show/invite/members`); web invite acceptance ([lib/controlkeel_web/live/invitation_live.ex](../lib/controlkeel_web/live/invitation_live.ex)); **NHI lifecycle audit** ([lib/controlkeel/platform/nhi_audit_event.ex](../lib/controlkeel/platform/nhi_audit_event.ex)) — `provisioned`/`token_rotated`/`deprovisioned`/`last_used_updated` event stream, `Platform.provision_agent_identity/3`, `revoke_agent_identity/2`, `rotate_agent_identity_token/2`, API `GET /service-accounts/:id/events`; **cross-workspace shared memory** — `visibility` field (`workspace`/`org`/`admin`) on `memory_records`, org-scoped search in `Store.Sqlite` via `org_id:` + `visibility:` opts |
| **Phase 5 — Cloud-agent runtime loop** | ✓ shipped | Run package + callback token ([lib/controlkeel/cloud/run_package.ex](../lib/controlkeel/cloud/run_package.ex), [lib/controlkeel/cloud/runtime_context.ex](../lib/controlkeel/cloud/runtime_context.ex)); CLI dispatch (`run cloud-agent <task-id> --runtime <runtime>`); HTTP callback ([lib/controlkeel_web/controllers/cloud_runtime_callback_controller.ex](../lib/controlkeel_web/controllers/cloud_runtime_callback_controller.ex)); Mission Control lifecycle on `/cloud/telemetry`; **behavioral baselining** ([lib/controlkeel/cloud/baseline_analyzer.ex](../lib/controlkeel/cloud/baseline_analyzer.ex)) — rolling 7-day per-tool means from `invocations` table, 3× deviation threshold auto-emits findings, CLI `baseline compute`, dashboard card on `/cloud/telemetry`; **provider fallback chain** ([lib/controlkeel/provider_broker/fallback_chain.ex](../lib/controlkeel/provider_broker/fallback_chain.ex)) — cost-ordered provider selection when budget blocks primary, CLI `provider set-fallback-chain`; **token amplification ratios** (`Budget.amplification_ratios/1`) — output/input ratio dashboard card + `ck_token_audit` mode `amplification` |
...
All previously deferred roadmap items are now implemented. The table below records each item with its implementing module for audit.

| Item | Phase | Implementing module(s) |
| --- | --- | --- |
| NATS JetStream durable consumers with replay | Phase 6 | [lib/controlkeel/bus/jet_stream.ex](../lib/controlkeel/bus/jet_stream.ex) — `Bus.JetStream`; plugged via `Runtime.bus_module/0` with `:jet_stream` config key; in-process `:queue` buffer when NATS unavailable |
| Air-gapped `.tar.gz` builder | Phase 6 | [lib/controlkeel/self_host.ex](../lib/controlkeel/self_host.ex) — `SelfHost.pack/2` via `:erl_tar` + SHA256; CLI `controlkeel selfhost pack` |
| EU AI Act + NIST AI RMF compliance templates | Phase 6 | [lib/controlkeel/cloud/compliance_template.ex](../lib/controlkeel/cloud/compliance_template.ex) — Art.9/10/13/14/17 + GOVERN/MAP/MEASURE/MANAGE sections alongside SOC 2 + GDPR |
| Behavioral baselining | Phase 5 | [lib/controlkeel/cloud/baseline_analyzer.ex](../lib/controlkeel/cloud/baseline_analyzer.ex) — rolling 7-day per-tool means, 3× deviation threshold auto-emits findings; CLI `baseline compute`; dashboard card on `/cloud/telemetry` |
| Signed/verified downstream MCP skill pipeline | Phase 3 | [lib/controlkeel/cloud/skill_attestation.ex](../lib/controlkeel/cloud/skill_attestation.ex) — Ed25519 cert verification via `:crypto`; `McpRegistry.lookup` accepts `cert:` opt for inline verification |
| Per-workspace scope-filtered tool catalogs | Phase 3 | [lib/controlkeel/accounts/workspace_tool_policy.ex](../lib/controlkeel/accounts/workspace_tool_policy.ex) — `inherit`/`allowlist`/`denylist` modes; gated in `authorize_hosted_tool_call`; CLI `workspace tool-policy get/set`; API `GET/PUT /workspaces/:id/tool-policy` |
| Non-human-identity (NHI) lifecycle management | Phase 4 | [lib/controlkeel/platform/nhi_audit_event.ex](../lib/controlkeel/platform/nhi_audit_event.ex) — `provisioned`/`token_rotated`/`deprovisioned`/`last_used_updated` event stream; `Platform.provision_agent_identity/3`, `revoke_agent_identity/2`, `rotate_agent_identity_token/2`; API `GET /service-accounts/:id/events`; CLI and API service-account commands route through NHI audit functions |
| Shared typed memory cross-workspace | Phase 4 | `visibility` field on `memory_records` (`workspace`/`org`/`admin`); `Store.Sqlite` `maybe_scope_visibility/2` supports org-level and admin-wide search via `org_id:` + `visibility:` opts |
| Provider fallback chains with cost-aware routing | Phase 5 | [lib/controlkeel/provider_broker/fallback_chain.ex](../lib/controlkeel/provider_broker/fallback_chain.ex) — cost-ordered provider selection when budget blocks primary; `Governor.preflight` returns `fallback_provider`; CLI `provider set-fallback-chain` |
| Amplification-ratio metric on dashboard | Phase 5 | `Budget.amplification_ratios/1` — output/input ratio per session; dashboard card on `/cloud/telemetry`; `ck_token_audit` mode `"amplification"` |
