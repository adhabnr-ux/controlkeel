...
| `ck_engineer_mirror` | Engineer-facing daily mirror: composes today's plans submitted, first-pass approvals, denials, 30-day prompt-quality outcomes (prompt_first_pass / prompt_refined_once / prompt_refined_repeatedly / prompt_abandoned), learned review patterns from the operator's decision history, and one top signal + suggestion. Read-only. Surfaces how the human is steering — not how the agent is performing. Pass session_id to inspect the current session's reflection view. |
...
| `ck_monitor_subscribe` | Subscribe to session events for remote monitoring via webhook. Provides read-only visibility into session activity. |
