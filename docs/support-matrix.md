...
| `ck_observability` | Read local observability reports for session runs, loop status, problems, memory, costs, trends, eval candidates, generated benchmark drafts/scenarios/history, regressions, and advisory promotion candidates. Read-only: no benchmark execution, draft approval, materialization, promotion, or autofix mutation. |
