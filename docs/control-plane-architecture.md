...
- **Relay handoffs over monolithic sessions**
  CK prefers relay-style execution where structured handoffs move between planner, worker, validator, and reviewer roles. The baton is not hidden chain-of-thought; it is explicit task state, blockers, evidence, and what remains. This keeps context bounded and reduces the chance that one long session drifts into repeated work or reward-hacking. CK handoff is a cross-agent, cross-runtime authority boundary change — the governed bundle (findings, proofs, explicit next-step state) transfers to a receiving agent with its own context window. This is distinct from host-managed compaction (e.g., Amp Neo auto-compaction), which provides same-agent context overflow relief without an authority boundary change. CK does not treat compaction as a substitute for handoff; they solve different problems.
