defmodule ControlKeel.UpdaterTest do
  ...
  test "sync-attached skips when a newer release is available and self update was not applied", %{
    tmp_dir: tmp_dir
  } do
    assert {:ok, _binding} =
             ProjectBinding.write(
    ...
    assert get_in(report, ["sync_result", "status"]) == "skipped"
    assert get_in(report, ["sync_result", "reason"]) == "self_update_required"
    assert get_in(report, ["sync_result", "message"]) =~ "Update ControlKeel first"
  end

  test "sync-attached asks for rerun after applying self update", %{tmp_dir: tmp_dir} do
    assert {:ok, _binding} =
             ProjectBinding.write(
               %{
                 "workspace_id" => 1,
    ...
    assert {:ok, report} =
             Updater.apply(tmp_dir,
               ...
               cmd_runner: fn
                 "brew", ["--prefix"], _opts -> {"/opt/homebrew\n", 0}
                 "brew", ["upgrade", "controlkeel"], _opts -> {"upgraded\n", 0}
                 _cmd, _args, _opts -> {"", 1}
               end
             )

    assert get_in(report, ["apply_result", "status"]) == "applied"
    assert get_in(report, ["sync_result", "status"]) == "skipped"
    assert get_in(report, ["sync_result", "reason"]) == "rerun_after_self_update"
  end
end
