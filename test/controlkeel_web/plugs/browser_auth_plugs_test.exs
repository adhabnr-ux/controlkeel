defmodule ControlKeelWeb.Plugs.BrowserAuthPlugsTest do
  ...
  setup %{conn: conn} do
    ...
    {:ok, user, membership} = Accounts.ensure_sso_membership(org.id, %{"email" => "user@example.com"}, role: "admin")
  ...
  test "LoadCurrentUser assigns user and active membership", %{conn: conn, user: user, membership: membership} do
