defmodule ControlKeel.Ops.DeploymentAdvisor do
  ...
  defp dockerfile(:react, root), do: react_dockerfile(root)
  ...
  defp docker_compose(:react, root), do: react_docker_compose(root)
  ...
  defp app_name(root) do
    case File.read(Path.join(root, "mix.exs")) do
      {:ok, content} ->
        case Regex.run(~r/app:\s*:([a-zA-Z0-9_]+)/, content) do
          [_, app] -> app
          _ -> root |> Path.basename() |> String.replace("-", "_")
        end

      {:error, _} ->
        root |> Path.basename() |> String.replace("-", "_")
    end
  end

  defp phoenix_uses_sqlite?(root) do
    file_contains?(root, "mix.exs", "ecto_sqlite3") or
      file_contains?(root, "mix.lock", "ecto_sqlite3")
  end

  defp otp_version(root) do
    case File.read(Path.join(root, ".tool-versions")) do
      {:ok, content} ->
        case Regex.run(~r/erlang\s+([0-9]+(?:\.[0-9]+)*)/, content) do
          [_, ver] -> ver
          _ -> "27.3.4.3"
        end
  ...
  defp phoenix_dockerfile(root) do
    ...
    ENV MIX_ENV=prod
    COPY --from=build --chown=nobody:root /app/_build/prod/rel/#{name} ./
    USER nobody
    ENV PHX_SERVER=true
    ENV LANG=C.UTF-8
  ...
  defp phoenix_docker_compose(root) do
    ...
    if phoenix_uses_sqlite?(root) do
      """
      # Generated by ControlKeel Deployment Advisor
      # Local-first Phoenix compose for SQLite-backed projects.
      services:
        app:
          build: .
          ports:
          ...
          environment:
            ...
            DATABASE_PATH: ${DATABASE_PATH:-/data/#{name}.db}
  ...
  defp phoenix_env(root) do
    ...
    if phoenix_uses_sqlite?(root) do
      """
      # Generated by ControlKeel Deployment Advisor
      # IMPORTANT: never commit real secrets. Use .env for local use and platform secrets for production.
      ...
      DATABASE_PATH=./#{name}_dev.db
      POOL_SIZE=10

      # Cloud/runtime mode only
  ...
  defp react_dockerfile(root) do
    if file_contains?(root, "package.json", "next") do
      """
      # Generated by ControlKeel Deployment Advisor
  ...
  defp react_docker_compose(root) do
    if file_contains?(root, "package.json", "next") do
      """
      # Generated by ControlKeel Deployment Advisor
      services:
