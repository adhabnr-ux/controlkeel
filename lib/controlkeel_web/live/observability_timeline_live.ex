  ...
  def render(assigns) do
    ...
    >
      <div class="flex items-start justify-between gap-4">
        <div>
          <h1 class="text-xl font-semibold text-[var(--primary)]">Timeline</h1>
      ...
      <div id="observability-timeline-summary" class="grid grid-cols-3 gap-4">
        <div class="rounded-xl p-4 border border-[var(--border)] bg-[rgba(255,255,255,0.015)] space-y-1">
          <p class="text-[var(--muted-foreground)] uppercase tracking-[0.1em] text-[10px]">Events</p>
          <p class="text-2xl font-semibold text-[var(--foreground)]">{@timeline.count}</p>
          <p class="text-[var(--muted-foreground)] text-xs">Limit {@timeline.limit}</p>
        </div>
        <div class="rounded-xl p-4 border border-[var(--border)] bg-[rgba(255,255,255,0.015)] space-y-1">
        ...
        <div class="rounded-xl p-4 border border-[var(--border)] bg-[rgba(255,255,255,0.015)] space-y-1">
          <p class="text-[var(--muted-foreground)] uppercase tracking-[0.1em] text-[10px]">Actors</p>
          <p class="text-2xl font-semibold text-[var(--foreground)]">
            {map_size(@timeline.by_actor)}
          </p>
          <p class="text-[var(--muted-foreground)] text-xs">{format_frequency(@timeline.by_actor)}</p>
        </div>
      </div>
              ...
              >
                <div class="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p class="text-sm font-semibold text-[var(--foreground)]">{event.event_type}</p>
                  </div>
                  <span class={neutral_pill_class()}>
                    {format_datetime(event.inserted_at, "unknown time")}
                  </span>
                </div>
                <p class="text-sm text-[var(--foreground)] leading-relaxed">{event.summary}</p>
                <%= if event.body not in [nil, ""] do %>
                  <p class="text-[var(--muted-foreground)] text-xs">{event.body}</p>
                <% end %>
              </div>
            <% end %>
          <% end %>
