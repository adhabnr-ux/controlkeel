  ...
  def render(assigns) do
    ...
    >
      <div class="flex items-start justify-between gap-4">
        <div>
          <h1 class="text-xl font-semibold text-[var(--primary)]">Costs</h1>
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="rounded-xl p-4 border border-[var(--border)] bg-[rgba(255,255,255,0.015)] space-y-1">
          ...
          <p class="text-2xl font-semibold text-[var(--foreground)]">{@costs.totals.invocations}</p>
          <p class="text-[var(--muted-foreground)] text-xs">{@costs.totals.sessions} session(s)</p>
        </div>
        <div class="rounded-xl p-4 border border-[var(--border)] bg-[rgba(255,255,255,0.015)] space-y-1">
          <p class="text-[var(--muted-foreground)] uppercase tracking-[0.1em] text-[10px]">
            Estimated spend
          ...
          <p class="text-[var(--muted-foreground)] text-xs">Local invocation estimate</p>
        </div>
        <div class="rounded-xl p-4 border border-[var(--border)] bg-[rgba(255,255,255,0.015)] space-y-1">
          <p class="text-[var(--muted-foreground)] uppercase tracking-[0.1em] text-[10px]">
            Input tokens
          </p>
          <p class="text-2xl font-semibold text-[var(--foreground)]">{@costs.totals.input_tokens}</p>
        ...
        <div class="rounded-xl p-4 border border-[var(--border)] bg-[rgba(255,255,255,0.015)] space-y-1">
          ...
          <p class="text-2xl font-semibold text-[var(--foreground)]">{@costs.totals.output_tokens}</p>
          <p class="text-[var(--muted-foreground)] text-xs">Across recorded calls</p>
        </div>
      </div>
      ...
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <%= for {grouping, costs} <- @grouped_costs do %>
            <div class="rounded-xl p-4 border border-[var(--border)] bg-[rgba(255,255,255,0.015)] space-y-3">
              <p class="text-[var(--muted-foreground)] uppercase tracking-[0.1em] text-[10px]">
                Grouped by {grouping}
                <span class="ml-2 text-[var(--primary)]">{length(costs.groups)} group(s)</span>
              </p>
              ...
              <% else %>
                <div class="space-y-2">
                  <%= for group <- costs.groups do %>
                    <div class="flex items-center justify-between gap-2 p-2 rounded-lg border border-[var(--border)] bg-[rgba(255,255,255,0.02)]">
                      <div>
                        <p class="text-sm font-medium text-[var(--foreground)]">{group.name}</p>
                        <p class="text-[var(--muted-foreground)] text-xs">
