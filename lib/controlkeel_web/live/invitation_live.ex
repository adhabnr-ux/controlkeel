defmodule ControlKeelWeb.InvitationLive do
      ...
      <section class="relative z-10 mx-auto flex min-h-screen w-full max-w-md flex-col justify-center px-4 py-16 sm:px-6 lg:px-8">
        <%= if !(@current_user && !@email_matches) do %>
          <div class="mb-8 text-center">
            <p class="mb-2 text-xs font-semibold uppercase tracking-[0.32em] text-lime-300">
              Invitation
            </p>
            <h1 class="text-3xl font-semibold tracking-tight text-white">Join {@org.name}</h1>
            <p class="mt-3 text-sm text-slate-400">
              You've been invited as
              <span class="font-semibold text-lime-300">{@membership.role}</span>
            </p>
          </div>
        <% end %>
        ...
        <div class="overflow-hidden rounded-[2rem] border border-white/10 bg-slate-900/95 p-8 shadow-[0_45px_120px_-40px_rgba(0,0,0,0.9)]">
          <%= cond do %>
            <% @current_user && @email_matches -> %>
              <div class="mb-6 flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 p-4">
                <span class="flex size-10 shrink-0 items-center justify-center rounded-full bg-emerald-400/15">
                  <.icon name="hero-check-circle" class="size-5 text-emerald-300" />
                </span>
                <div class="min-w-0">
                  <p class="text-sm font-medium text-white">Signed in</p>
                  <p class="truncate text-sm text-slate-400">{@current_user.email}</p>
                </div>
              </div>
              <button
                type="button"
                phx-click="accept"
                class="flex w-full items-center justify-center gap-2 rounded-2xl bg-lime-300 px-5 py-3 text-sm font-semibold text-zinc-950 shadow-lg shadow-lime-300/20 transition hover:-translate-y-0.5 hover:bg-lime-200"
              >
                <.icon name="hero-check" class="size-5" /> Accept invitation
              </button>
              <a
                href={~p"/dashboard"}
                class="mt-3 flex w-full items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-5 py-3 text-sm font-semibold text-white transition duration-200 hover:-translate-y-0.5 hover:bg-white/15"
              >
                <.icon name="hero-arrow-left" class="size-4" /> Back to dashboard
              </a>
            <% @current_user && !@email_matches -> %>
              <div class="mb-6 flex items-center gap-3 rounded-2xl border border-red-500/20 bg-red-500/5 p-4">
                <span class="flex size-10 shrink-0 items-center justify-center rounded-full bg-red-400/15">
                  <.icon name="hero-exclamation-triangle" class="size-5 text-red-300" />
                </span>
                <div class="min-w-0">
                  <p class="text-sm font-medium text-white">Email mismatch</p>
                  <p class="mt-0.5 text-sm text-slate-400">
                    Invite sent to <code class="text-lime-300">{@invited_user.email}</code>
                  </p>
                  <p class="text-sm text-slate-400">
                    You're signed in as <code class="text-red-300">{@current_user.email}</code>
                  </p>
                </div>
              </div>
              <div class="mt-4 flex gap-3">
                <a
                  href={~p"/dashboard"}
                  class="flex flex-1 items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-5 py-3 text-sm font-semibold text-white transition duration-200 hover:-translate-y-0.5 hover:bg-white/15"
                >
            ...
            <% true -> %>
              <div class="mb-6 rounded-2xl border border-white/10 bg-white/5 p-4 text-center">
                <p class="text-sm text-slate-400">
                  This invitation was issued to
                </p>
                <p class="mt-1 font-mono text-sm text-lime-300">{@invited_user.email}</p>
              </div>
  ...
  def render(%{state: :already_accepted} = assigns) do
    ...
    <div class="min-h-screen bg-slate-950 text-white">
      ...
      <section class="relative z-10 mx-auto flex min-h-screen w-full max-w-md flex-col justify-center px-4 py-16 sm:px-6 lg:px-8">
        <div class="overflow-hidden rounded-[2rem] border border-white/10 bg-slate-900/95 p-8 text-center shadow-[0_45px_120px_-40px_rgba(0,0,0,0.9)]">
          <div class="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-3xl border border-white/10 bg-white/5">
            <.icon name="hero-check-badge" class="size-7 text-slate-400" />
          </div>
          <h1 class="text-2xl font-semibold tracking-tight text-white">Already accepted</h1>
          <p class="mt-3 text-sm text-slate-400">
          ...
          <a
            ...
            class="mt-6 inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-5 py-3 text-sm font-semibold text-white transition duration-200 hover:-translate-y-0.5 hover:bg-white/15"
          >
  ...
  def render(%{state: :invalid} = assigns) do
    ...
    <div class="min-h-screen bg-slate-950 text-white">
      <div class="pointer-events-none absolute inset-0 overflow-hidden">
        <div class="absolute left-1/2 top-8 h-80 w-80 -translate-x-1/2 rounded-full bg-red-400/10 blur-3xl" />
      </div>

      <section class="relative z-10 mx-auto flex min-h-screen w-full max-w-md flex-col justify-center px-4 py-16 sm:px-6 lg:px-8">
        <div class="overflow-hidden rounded-[2rem] border border-white/10 bg-slate-900/95 p-8 text-center shadow-[0_45px_120px_-40px_rgba(0,0,0,0.9)]">
          <div class="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-3xl border border-white/10 bg-white/5">
            <.icon name="hero-x-circle" class="size-7 text-red-400" />
          </div>
          <h1 class="text-2xl font-semibold tracking-tight text-white">Invitation not found</h1>
          <p class="mt-3 text-sm text-slate-400">
  ...
  def render(%{state: :error} = assigns) do
    ...
    <div class="min-h-screen bg-slate-950 text-white">
      ...
      <section class="relative z-10 mx-auto flex min-h-screen w-full max-w-md flex-col justify-center px-4 py-16 sm:px-6 lg:px-8">
        <div class="overflow-hidden rounded-[2rem] border border-white/10 bg-slate-900/95 p-8 text-center shadow-[0_45px_120px_-40px_rgba(0,0,0,0.9)]">
          <div class="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-3xl border border-white/10 bg-white/5">
            <.icon name="hero-exclamation-triangle" class="size-7 text-red-400" />
          </div>
          <h1 class="text-2xl font-semibold tracking-tight text-white">Something went wrong</h1>
          <p class="mt-3 text-sm text-slate-400">
