defmodule ControlKeelWeb.ObservabilityPromotionsLive do
  ...
  def render(assigns) do
    ...
    >
      <div class="flex items-start justify-between gap-4">
        <div>
          <h1 class="text-xl font-semibold text-[var(--primary)]">Promotion candidates</h1>
      ...
      <div class="rounded-xl p-4 border border-[var(--border)] bg-[rgba(255,255,255,0.015)] space-y-1">
        ...
        <%= if @promotions.recommendations == [] do %>
          <p class="text-[var(--muted-foreground)] text-sm">No promotion recommendations yet.</p>
        <% else %>
          <ul class="space-y-2 list-disc pl-4">
            <%= for recommendation <- @promotions.recommendations do %>
              <li class="text-[var(--foreground)] text-sm leading-relaxed">{recommendation}</li>
            <% end %>
          </ul>
        <% end %>
      </div>
      ...
      <div class="space-y-3">
        ...
        <%= if @promotions.candidates == [] do %>
          <p class="text-[var(--muted-foreground)] text-sm">No promotion candidates yet.</p>
        <% else %>
          <%= for candidate <- @promotions.candidates do %>
            ...
            >
              <div class="flex items-center justify-between gap-4">
                <p class="text-sm font-semibold text-[var(--foreground)]">{candidate.rule_id}</p>
                <span class={readiness_pill_class(candidate.readiness)}>{candidate.readiness}</span>
              </div>
              <p class="text-[var(--muted-foreground)] text-xs">{candidate.suggested_action}</p>
            </div>
          <% end %>
        <% end %>
      </div>
  ...
  defp readiness_pill_class("ready"),
    do:
      "inline-flex items-center border border-[var(--border)] rounded-full px-2.5 py-1 text-xs bg-[rgba(125,226,174,0.1)] text-[#d2ffe7]"
