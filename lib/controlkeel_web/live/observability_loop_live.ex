  ...
  def render(assigns) do
    ...
    >
      <div class="flex items-start justify-between gap-4">
        <div>
          <h1 class="text-xl font-semibold text-[var(--primary)]">Learning loop</h1>
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div class="rounded-xl p-4 border border-[var(--border)] bg-[rgba(255,255,255,0.015)] space-y-1">
          <p class="text-[var(--muted-foreground)] uppercase tracking-[0.1em] text-[10px]">Evals</p>
          <p class="text-lg font-semibold text-[var(--foreground)]">
      ...
      <div class="space-y-3">
        ...
        <%= if @loop.blockers == [] do %>
          <p class="text-[var(--muted-foreground)] text-sm">No learning-loop blockers detected.</p>
        <% else %>
          <div class="space-y-2">
            <%= for blocker <- @loop.blockers do %>
              <div class="rounded-xl px-4 py-3 border border-[var(--border)] bg-[rgba(255,255,255,0.015)]">
                <p class="text-sm font-semibold text-[var(--foreground)]">{blocker.id}</p>
                <p class="text-[var(--muted-foreground)] text-xs">{blocker.reason}</p>
              </div>
            <% end %>
          </div>
        <% end %>
      ...
      <div class="space-y-3">
        ...
        <%= if @loop.next_actions == [] do %>
          <p class="text-[var(--muted-foreground)] text-sm">No next actions.</p>
        <% else %>
          <div class="space-y-2">
            <%= for action <- @loop.next_actions do %>
              <div class="rounded-xl px-4 py-3 border border-[var(--border)] bg-[rgba(255,255,255,0.015)] space-y-1">
                <p class="text-sm font-semibold text-[var(--foreground)]">
                  [{action.priority}] {action.title}
                </p>
                <p class="text-[var(--muted-foreground)] text-xs">{action.suggested_action}</p>
              </div>
            <% end %>
          </div>
        <% end %>
