  ...
  def render(assigns) do
    ...
    >
      <div class="flex items-start justify-between gap-4">
        <div>
          <h1 class="text-xl font-semibold text-[var(--primary)]">Benchmark history</h1>
      ...
      <div
        ...
        class="rounded-xl px-4 py-3 border border-[var(--border)] bg-[rgba(255,255,255,0.015)] space-y-4"
      >
        <div class="space-y-1">
          <p class="text-[var(--muted-foreground)] uppercase tracking-[0.1em] text-[10px]">
            Readiness
          </p>
          <p class="text-base font-semibold text-[var(--foreground)]">{@history.readiness.reason}</p>
        </div>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div class="rounded-lg p-3 border border-[var(--border)] bg-[rgba(255,255,255,0.02)] space-y-1">
            <p class="text-[var(--muted-foreground)] uppercase tracking-[0.1em] text-[10px]">
              Saved evals
            </p>
      ...
      <div id="observability-benchmark-history-runs" class="space-y-3">
        ...
        <%= if @history.runs == [] do %>
          <p class="text-[var(--muted-foreground)] text-sm">No observability benchmark runs yet.</p>
        <% else %>
