...
```text
Set up ControlKeel for this repository. Follow README.md plus docs/getting-started.md, docs/direct-host-installs.md, docs/support-matrix.md, and docs/agent-integrations.md. Install CK if missing, run controlkeel setup, attach the strongest useful host path, then run attach doctor, provider doctor, status, findings, controlkeel me, and the host MCP check. Apply only safe local fixes, redact secrets, and pause for trust, provider, restart, or review-approval steps.
```
